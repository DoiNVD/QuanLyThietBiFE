...
const DonViTraLuongForm = ({ history, match, permission }) => {
  ...
  const getInfo = () => {
    ...
    new Promise((resolve, reject) => {
      dispatch(
        fetchStart(`DonViTraLuong/${id}`, "GET", null, "DETAIL", "", resolve, reject)
      );
    })
      .then((res) => {
        if (res && res.data) {
  ...
  const saveData = (user, saveQuit = false) => {
    if (type === "new") {
      ...
      new Promise((resolve, reject) => {
        dispatch(
          fetchStart(`DonViTraLuong`, "POST", newData, "ADD", "", resolve, reject)
        );
      })
        .then((res) => {
          if (saveQuit) {
    ...
    if (type === "edit") {
      ...
      new Promise((resolve, reject) => {
        dispatch(
          fetchStart(`DonViTraLuong/${id}`, "PUT", newData, "EDIT", "", resolve, reject)
        );
      })
        .then((res) => {
          if (saveQuit) {
  ...
  const formTitle = type === "new" ? "Thêm mới đơn vị trả lương" : "Chỉnh sửa đơn vị trả lương";
  return (
    <div className="gx-main-content">
      ...
      <Card className="th-card-margin-bottom">
        ...
        >
          ...
          >
            <Input className="input-item" placeholder="Nhập mã đơn vị trả lương" />
          </FormItem>
          <FormItem
          ...
          >
            <Input className="input-item" placeholder="Nhập tên đơn vị trả lương" />
          </FormItem>
          <FormSubmit
            goBack={goBack}
